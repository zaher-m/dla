#!/usr/bin/env python3
"""The PDF Structural Reference: what one page's own content stream implies.

Text lines and blocks, the graphic and ruled-table areas that hold them, the
gutters and column bands, and the body font size -- all in the pixel space of
the page's render, so geometry from a model and geometry from the PDF land in
the same coordinates and can be compared without an alignment step.

This is not ground truth.  It is a deterministic reading of the file, and it is
wrong in known ways: a table with shaded cell fills reads as a graphic area.
What it is instead is a reference that exists for every born-digital page at no
cost, needs no annotation, and never disagrees with itself.

The module depends on PyMuPDF and numpy and on nothing else in this repository,
including nothing under `core`.  That is deliberate and enforced by
docker/validation.Dockerfile, which builds this package alone: validation is
the library other software is meant to build on, and a library that drags a
benchmark harness and a CUDA image behind it is not one.
"""
